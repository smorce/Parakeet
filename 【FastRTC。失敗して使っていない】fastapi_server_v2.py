...
class Offer(BaseModel):
    """WebRTCオファー/アンサーのSDPとタイプを定義するPydanticモデル。"""
    sdp: str
    type: str
    webrtc_id: str   # これは重要なので消さないで。FastRTC のリリース履歴を見ると、バージョン 0.0.23（2025-04-23）以前には webrtc_id がモデルに含まれておらず、0.0.24（2025-05-13）以降で追加されています 
...
# --- リアルタイム音声処理ハンドラ ---
async def handle_audio_realtime(audio: tuple[int, np.ndarray]):
    """
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_file:
            ...
            # sf.writeはディスクI/Oを伴うブロッキング処理のため、スレッドプールで実行
            await run_in_threadpool(sf.write, tmp_path, audio_data, sr)
        
        logger.info(f"Transcribing audio chunk from {tmp_path}...")
        if model_device == "cuda": torch.cuda.empty_cache()

        # nemo-asrのtranscribeメソッドは同期的で重い処理のため、スレッドプールで実行
        transcriptions = await run_in_threadpool(model.transcribe, [tmp_path])
...
async def handle_audio_clone(audio: tuple[int, np.ndarray]):
    """
    ボイスクローニング用の音声処理ハンドラ。
    ...
    try:
        ...
        # model.transcribeは同期的で重い処理のため、スレッドプールで実行
        transcriptions = await run_in_threadpool(model.transcribe, [tmp_path])
        if transcriptions and isinstance(transcriptions, list) and len(transcriptions) > 0:
...
# --- FastRTCのエンドポイントを手動で定義 ---
# FastRTCの stream.mount() が内部で行う処理の一部を再現します。
# offer エンドポイントを自前で定義し、クライアント側で発生する 'type' is null エラーを解決します。
@app.post("/webrtc/offer")
async def webrtc_offer(offer: Offer):
    """WebRTCのオファーを受け取り、アンサーを返します。"""
    try:
        answer = await stream.offer(offer)
        # stream.offer()は辞書を返すため、そのまま返却する
        return answer
    except Exception as e:
        logger.error(f"Error processing /webrtc/offer: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error in webrtc_offer: {str(e)}")

@app.post("/clone/webrtc/offer")
async def clone_webrtc_offer(offer: Offer):
    """ボイスクローニング用のWebRTCオファーを受け取り、アンサーを返します。"""
    try:
        answer = await stream_clone.offer(offer)
        # stream_clone.offer()は辞書を返すため、そのまま返却する
        return answer
    except Exception as e:
        logger.error(f"Error processing /clone/webrtc/offer: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error in clone_webrtc_offer: {str(e)}")
