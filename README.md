...
| 項目 | ファイル処理 | マイク処理 |
|------|-------------|------------|
| **エンジン** | NVIDIA Parakeet-TDT-CTC | Google Web Speech API |
| **動作場所** | ローカル（GPU/CPU） | クラウド |
| **モデル読み込み** | 必要（重い処理） | 不要 |
| **メモリ使用** | 大量（GPU/CPUメモリ） | 最小限 |
| **長時間音声** | 分割処理対応（30秒単位） | 短時間推奨 |
| **オフライン** | 可能 | 不可（インターネット必須） |
| **処理速度** | デバイス依存 | ネットワーク依存 |
| **プライバシー** | 完全ローカル | 音声がGoogleに送信 |
